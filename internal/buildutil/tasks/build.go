...
func Build(verbose bool) error {
	ui.Section("Building Guild Scaffold")
	...
	cmd := exec.Command("go", "build", "-o", "bin/scaffold", "./cmd/scaffold")
	if verbose {
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
	}
...
// BuildOnly builds the main scaffold binary without running go vet (fast user installation)
func BuildOnly(verbose bool) error {
	...
	ui.Task("Building", "scaffold binary")
	...
	if err := cmd.Run(); err != nil {
		ui.TaskFail()
		return fmt.Errorf("failed to build scaffold binary: %w", err)
	}
